...
from mycurrency.currency_converter.serializers import GetCurrencyConverterFilter, GetCurrencyConverterSerializer, \
    validate_fields
...
class CurrencyConverterView(ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if validation is None:
            ...
            queryset = get_latest_currency_exchange_rate(source_currency, exchanged_currency)
            if queryset.count() <= 0:
                ...
                return Response(
                    {"error": f'An error occurred when retrieving latest exchange rate. Try it again later. '
                              ...
                              f'possible information to replicate the error.'}, status=500)
